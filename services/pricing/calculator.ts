...
export function getPricePerPlayer(selectedOption: PricingCardProps['option'], playerCount: number) {    
...
export function calculateFlexiblePrice(selectedOption: PricingCardProps['option'], players: string): string {
    const playerCount = parseInt(players) || 1;
    const pricePerPlayer = getPricePerPlayer(selectedOption, playerCount);
    return Math.ceil(((pricePerPlayer * playerCount) / 5) * 5).toFixed(2);
}
